...
class RDBCell(nn.Module):
    def __init__(self, para):
        ...
        self.in_channels = para.in_channels
        self.F_B0 = conv5x5(self.in_channels, self.n_feats, stride=1)
...
class Reconstructor(nn.Module):
    def __init__(self, para):
        ...
        self.model = nn.Sequential(
            ...
            conv5x5(self.n_feats, para.in_channels, stride=1)
        )
...
class ESTRNN(nn.Module):
    ...
    def __init__(self, args):
        ...
        para.in_channels = args.inChans  # Now you can add new keys
        para.past_frames = args.numFrames
        para.future_frames = 0
        OmegaConf.set_struct(para, True)
        self.para = para
        self.n_feats = para.n_features
        self.num_ff = para.future_frames
        self.num_fb = para.past_frames
        ...
        if para.reconstruction == 'interpolation':
            self.recons = ReconstructorInterpolate(para)
        elif para.reconstruction == 'original':
